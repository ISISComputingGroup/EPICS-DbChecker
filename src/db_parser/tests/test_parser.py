...
class MockLexer:
    ...
    def add_key_value_pair(self, key, value):
        ...
        """
        return (
            self.add_token(TokenTypes.L_BRACKET)
    ...
    def add_alias_field(self, value):
        return (
            self.add_token(TokenTypes.ALIAS)
    ...
    def add_macro(self):
        return (
            self.add_token(TokenTypes.BRACKET_MACRO_START)
...
class ParserTests(unittest.TestCase):
    ...
    def test_WHEN_parser_given_a_quoted_string_THEN_parser_can_extract_the_value(self):
        ...
        lexer = MockLexer().add_token(TokenTypes.QUOTED_STRING, '"{}"'.format(val))
    ...
    def test_WHEN_parser_given_a_string_literal_THEN_parser_can_extract_the_value(self):
        ...
        lexer = MockLexer().add_token(TokenTypes.LITERAL, "{}".format(val))

        self.assertEqual(Parser(lexer).value(), val)

    def test_GIVEN_a_token_that_does_not_represent_a_value_WHEN_parse_value_THEN_parser_error(self):
        lexer = MockLexer().add_token(TokenTypes.RECORD)
    ...
    def test_GIVEN_a_bracketed_key_value_pair_where_comma_is_missing_WHEN_parse_key_value_pair_THEN_raises_error(
        self,
    ):
        key, value = "Key", "Value"
    ...
    def test_GIVEN_a_field_declaration_WHEN_parse_field_THEN_can_extract_key_and_value(self):
        ...
        # field(PINI, "YES")
        lexer = MockLexer().add_token(TokenTypes.FIELD).add_key_value_pair(key, value)
    ...
    def test_GIVEN_an_empty_record_declaration_WHEN_parse_record_THEN_parser_can_extract_the_information(
        self,
    ):
    ...
    def test_GIVEN_a_record_declaration_containing_a_field_and_an_info_WHEN_parse_record_THEN_parser_can_extract_the_information(
        ...
        self.assertEqual(parsed_record.get_type(), rec_type)
        self.assertEqual(
            [field.unpack() for field in parsed_record.fields], [(field_name, field_val)]
        )
    ...
    def test_GIVEN_multiple_records_with_no_bodies_WHEN_parse_as_db_THEN_parser_can_extract_the_information(
        self,
    ):
        rec_type_1 = "ai"
        rec_type_2 = "bi"
    ...
    def test_GIVEN_record_containing_macros_WHEN_parse_record_THEN_parser_can_find_fields_with_starting_macro(
        self,
    ):
        rec_type = "ai"
        rec_name = "$(P)TEST1"
