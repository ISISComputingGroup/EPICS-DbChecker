...
def generate_single_record(original_record, name):
    return textwrap.dedent(
        ...
        """.format(
            ...
            name,
            add_waveform_specfic(original_record.get_nelm(), original_record.get_ftvl()),
        )
    )
...
def generate_sim_records(records, sim_record_name, dis_record_name):
    ...
    for g in groups.keys():
        ...
        if (
            records[groups[g].main].get_dtyp() is None
            or records[groups[g].main].get_dtyp().lower() == "soft channel"
        ):
            continue
        ...
        if groups[g].RB != sim_record_name and groups[g].RB != dis_record_name:
            ...
            if typ in ALLOWED_SIM_TYPES:
                ...
                output += generate_record_text(
                    records[groups[g].main], groups[g].RB, groups[g].SP, groups[g].SP_RBV
                )

    return output


def generate_modifed_db(
    file_in_path, db, file_out_path="generated.db", insert_sims=True, insert_disable=True
):
    ...
    with open(file_in_path, "r") as in_file, open(file_out_path, "w") as out_file:
        ...
        for line in in_file:
            ...
            if matched_pv is not None:
                ...
                if re.match("}$", line.strip()) is not None:
                    ...
                    if (
                        curr_record.type in ALLOWED_SIM_TYPES
                        and curr_record.get_dtyp() is not None
                        and curr_record.get_dtyp().lower() != "soft channel"
                    ):
        ...
        if insert_sims:
            new_records = generate_sim_records(records_dict, sim_record_name, dis_record_name)
